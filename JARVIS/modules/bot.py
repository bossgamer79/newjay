...
async def ping(e):
    if e.sender_id in SUDO_USERS:
        ...
        await jarvis.edit(f"[𝐘𝐎𝐔𝐑 𝐃𝐀𝐃 𝐈𝐒 𝐁𝐀𝐂𝐊 𝐁𝐑𝐎 𝐅𝐔𝐂𝐊 𝐘𝐎𝐔 𝐇𝐀𝐓𝐄𝐑𝐒](https://t.me/Officialjay_store)[𝐁𝐀𝐁𝐀𝐘🥀](https://t.me/Officialjay_store)🤖\n» `{mp} ᴍꜱ`")
...
async def addsudo(event):
    if event.sender_id == OWNER_ID:
        ...
        ok = await event.reply(f"» __Jay Ka Ek Beta Aur Add Ho rha hai..__")
        target = ""
        ...
        await event.reply("» BSDK SIRF JAY SUDO DE SKTA HAI...")
...
async def removesudo(event):
    if event.sender_id == OWNER_ID:
        Heroku = heroku3.from_key(HEROKU_API_KEY)
        sudousers = getenv("SUDO_USERS", default=None)
        ok = await event.reply(f"YE Jay Ki Najayaz Aulad thi isiliye nikal diya💋...")
        target = ""
        if HEROKU_APP_NAME is not None:
            app = Heroku.app(HEROKU_APP_NAME)
        else:
...
async def show_sudo_users(event):
    if event.sender_id == OWNER_ID:
        sudo_users_list = "Jay Ke Bache hai ye:\n"
        for user_id in SUDO_USERS:
            sudo_users_list += f"- {user_id}\n"
        await event.reply(sudo_users_list)
    else:
        await event.reply("Only Jay view the sudo users list.")
